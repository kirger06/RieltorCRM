    ...
    public class PublicController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 12)
        {
            ...
            if (companyId.HasValue)
            {
                var agentIds = await _context.Users
                    .Where(u => u.CompanyId == companyId.Value && u.Role == UserRole.Agent)
                    .Select(u => u.Id)
                    .ToListAsync();
                query = query.Where(p => p.AgentId.HasValue && agentIds.Contains(p.AgentId.Value));
            }
        ...
        public async Task<IActionResult> GetProperty(int id)
        {
            ...
            var property = new
            {
                ...
                AgentPhone = raw.Agent?.PhoneNumber,
        ...
        public async Task<IActionResult> GetCompanies()
        {
            var companies = await _context.Users
                .Where(u => u.Role == UserRole.Agent && u.CompanyId.HasValue)
                .Select(u => u.CompanyId!)
                .Distinct()
                .Join(_context.Companies, id => id, c => (int?)c.Id, (id, c) => new { c.Id, c.Name })
                .OrderBy(c => c.Name)
                .ToListAsync();
