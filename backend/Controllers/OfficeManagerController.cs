    ...
    public class OfficeManagerController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            ...
            if (!string.IsNullOrWhiteSpace(status) &&
                Enum.TryParse<PropertyStatus>(status, true, out var parsedStatus))
            {
                allProps = allProps.Where(p => p.Status == parsedStatus).ToList();
            }

            var total = allProps.Count;
            var props = allProps
                .Skip((page - 1) * pageSize)
                .Take(pageSize)
                .Select(p => new
            ...
            return Ok(new { total, page, pageSize, items = props });
        }
